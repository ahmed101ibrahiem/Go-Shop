...
class ProductProvider extends ChangeNotifier{
  ...
  List<ProductModel> searchProducts({required String searchText,required List<ProductModel> specificProducts}){
    List<ProductModel> searchList = specificProducts.where((element) =>
  ...
  List<ProductModel> findByCategory({required String ctgName}){
    List<ProductModel> products = _localProds.where((element) =>
    element.productCategory.toLowerCase().contains(ctgName.toLowerCase())).toList();
  return products;
  }
 ProductModel? findByProdId(String productId){
   if(_localProds.where((element) => element.productId == productId).isEmpty){
     return null;
   }
    return _localProds.firstWhere((element) => element.productId == productId);
  }
  List<ProductModel> get getProducts{
    return _localProds;
  }
  final List<ProductModel> _localProds = [
    // Phones
    ProductModel(
      //1
